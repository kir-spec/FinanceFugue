...
class ToggleSwitch(QWidget):
    ...
    def __init__(self, parent=None, width=120, height=28):
        super().__init__(parent)
    ...
    def paintEvent(self, event):
        ...
        if opacity_off > 0.1:
            ...
            # Actually, text should be on the right side when knob is left
            off_text_rect = QRectF(current_x + knob_diameter, 0, w - (current_x + knob_diameter), h)
            painter.drawText(off_text_rect, Qt.AlignmentFlag.AlignCenter, "НЕ ВЫПОЛНЕН")
        ...
        if opacity_on > 0.1:
            ...
            painter.drawText(on_text_rect, Qt.AlignmentFlag.AlignCenter, "ВЫПОЛНЕН")
...
class ClickableStatBox(QFrame):
    ...
    def __init__(self, title, value, color, check_debt=False, parent=None):
        super().__init__(parent)
        self.setCursor(Qt.CursorShape.PointingHandCursor)
        self.setStyleSheet("""
            QFrame {
...
class ClientStatsWidget(QFrame):
    ...
    def init_ui(self, stats):
        ...
        self.sum_box = ClickableStatBox("СУММА ЗАКАЗОВ", stats['total_sum'], "#00D1FF")
        self.sum_box.clicked.connect(self.sum_clicked)
        
        self.paid_box = ClickableStatBox("ВСЕГО ОПЛАЧЕНО", stats['total_received'], "#28A745")
        self.paid_box.clicked.connect(self.paid_clicked)

        self.adv_box = ClickableStatBox("АВАНСЫ", stats['total_advance'], "#FFD700")
        self.adv_box.clicked.connect(self.adv_clicked)

        self.debt_box = ClickableStatBox("ДОЛГ", stats['total_debt'], "#FF4B2B", check_debt=True)
...
class OrderWidget(HoverShadowFrame):
    ...
    def init_ui(self):
        ...
        # Тумблер "Выполнен"
        self.status_switch = ToggleSwitch(width=110, height=26)
