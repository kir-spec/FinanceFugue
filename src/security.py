...
class SecurityManager:
    ...
    def setup_encryption(self, db_password: Optional[str] = None):
        """Включение шифрования. Если пароль не передан, используется внутренний ключ без хеширования пароля."""
        if not self.key or not os.path.exists("crm.key"):
        ...
        if db_password:
            ...
            self.salt = None # Для шифрования без пароля, когда используется внутренний ключ
    ...
    def unlock(self, password: str) -> bool:
        """Попытка разблокировки (приложения и/или базы данных) с помощью пароля."""
        # 1. Проверка пароля приложения, если он установлен.
        if self.has_app_password():
            if not self.check_app_password(password):
                return False
        
        # 2. Если база зашифрована, пытаемся разблокировать ее ключ.
        if self.is_encrypted():
            # Если для шифрования базы установлен пароль (self.password_hash),
            # то переданный `password` должен совпадать с ним.
            if self.password_hash and not self.check_db_password(password):
                return False
            
            # Загружаем ключ шифрования. Если ключ не загружен или отсутствует,
            # то разблокировка невозможна.
            if os.path.exists("crm.key"):
                try:
                    with open("crm.key", "rb") as f:
                        self.key = f.read()
                    ...
                    # Ошибка чтения ключа или неверный формат
                    return False
            else:
                # Если ключ должен быть, но его нет - ошибка.
                return False
                
        return True # Успешная разблокировка
    ...
    def _save_security_config(self):
        ...
        if self.salt:
            ...
            # Удаляем ключ, если salt равен None
            settings.pop("security_salt", None)
            
        if self.password_hash:
            settings["password_hash"] = self.password_hash
        else:
            # Удаляем ключ, если password_hash равен None
            settings.pop("password_hash", None)
            
        if self.app_salt:
            settings["app_salt"] = base64.b64encode(self.app_salt).decode()
        else:
            # Удаляем ключ, если app_salt равен None
            settings.pop("app_salt", None)
            
        if self.app_hash:
            settings["app_hash"] = self.app_hash
        else:
            # Удаляем ключ, если app_hash равен None
            settings.pop("app_hash", None)
