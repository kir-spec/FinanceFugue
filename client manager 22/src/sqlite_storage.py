    ...
    def add_client(self, client: Client) -> bool:
        ...
        try:
            with self._get_connection() as conn:
                ...
                """, (
                    client.id,
                    self._encrypt(client.name),
                    self._encrypt(client.notes),
                    self._encrypt(client.email),
                    self._encrypt(client.telegram),
                    self._encrypt(client.vk),
                    self._encrypt(client.facebook),
                    self._encrypt(client.social_link)
                ))
                conn.commit()
                return True
        except sqlite3.Error as e:
            logger.error(f"SQLite: Error adding client: {e}")
            return False
