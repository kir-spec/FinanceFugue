...
class CRMBackend(QObject):
    ...
    def init_security_check(self):
        if not self.app_settings.get('first_run_completed', False):
            ...
            self._load_data()
            # Отправляем сигнал чтобы main_window обновил UI
            self.securityRequest.emit("data_loaded")
    ...
    def unlock_database(self, password: str) -> bool:
        """Разблокировка базы данных паролем (для приложения)"""
        # Если база зашифрована, сначала пытаемся разблокировать ее.
        # Если не зашифрована, то проверяем только пароль приложения.
        if self.security.is_encrypted():
            if not self.security.unlock(password): # Разблокировка шифрованной базы
                return False
        
        # После разблокировки шифрования, или если база не была зашифрована,
        # проверяем пароль приложения (если он есть).
        if self.security.has_app_password():
            if not self.security.check_app_password(password):
                return False
    ...
    def setup_initial_config(self, db_path: str, file_storage_mode: str, encryption_enabled: bool, app_password: Optional[str]) -> bool:
        ...
        try:
            ...
            # Отдельная установка пароля для входа, если он есть (независимо от шифрования)
            if app_password:
    ...
    def check_password(self, password: str) -> bool:
        ...
        return self.security.check_app_password(password)
    ...
    def add_client(self, name: str) -> bool:
        logger.info(f"add_client вызван с именем: '{name}'")
        
        if not name.strip():
            logger.warning("Попытка добавить клиента с пустым именем")
            self.errorMessage.emit("Введите имя клиента")
            return False
        
        if any(c.name.lower() == name.strip().lower() for c in self._clients):
            logger.warning(f"Клиент с именем '{name}' уже существует")
        ...
        logger.info(f"Создан объект клиента: ID={new_client.id}, name='{new_client.name}'")
        
        result = self.sqlite.add_client(new_client)
        logger.info(f"sqlite.add_client вернул: {result}")
        
        if result:
            self._clients.append(new_client)
            self.clients_model.set_clients(self._clients)
            self.save_database()
            logger.info(f"Успешно добавлен клиент: {new_client.name} (ID: {new_client.id})")
            self.successMessage.emit(f"Клиент '{name}' успешно создан")
            return True
        else:
            logger.error(f"Ошибка при сохранении клиента '{name}' в базу данных")
    ...
    def add_client_obj(self, client_obj: Client) -> bool:
        """Добавляет объект клиента в БД."""
        if not client_obj.name.strip():
            self.errorMessage.emit("Введите имя клиента")
            return False
        
        if any(c.name.lower() == client_obj.name.strip().lower() for c in self._clients):
            logger.warning(f"Клиент с именем '{client_obj.name}' уже существует, используем существующего")
            # Возвращаем True, но не добавляем дубликат
            return True
        
        result = self.sqlite.add_client(client_obj)
        if result:
            self._clients.append(client_obj)
            self.clients_model.set_clients(self._clients)
            self.save_database()
            logger.info(f"Добавлен клиент: {client_obj.name} (ID: {client_obj.id})")
            return True
        else:
            logger.error(f"Ошибка сохранения клиента '{client_obj.name}' в базу данных")
            self.errorMessage.emit("Ошибка сохранения клиента в базе данных")
            return False
    ...
    def add_order_obj(self, client_id: str, new_order: Order) -> bool:
        client = self.clients_model.get_client_by_id(client_id)
        if not client:
            self.errorMessage.emit("Клиент не найден")
            return False
        ...
        if self.sqlite.add_order(client_id, new_order):
            ...
            self.save_database()
            logger.info(f"Добавлен заказ '{new_order.service_type}' для клиента {client.name}")
            self.successMessage.emit(f"Заказ '{new_order.service_type}' успешно создан")
            return True
        else:
            self.errorMessage.emit("Ошибка сохранения заказа в базе данных")
            return False
    ...
    def import_json(self, path: str) -> bool:
        """
        Импортирует данные из JSON или ZIP архива.
        При импорте ZIP, полностью заменяет текущую базу данных и файлы.
        При импорте JSON, только обновляет данные в БД.
        """
        try:
            # --- Обработка ZIP-архива (полное восстановление) ---
            if path.lower().endswith('.zip'):
                return self._import_from_zip(path)

            # --- Обработка JSON-файла (только данные) ---
            return self._import_from_json(path)

        except Exception as e:
            logger.error(f"Ошибка импорта файла '{path}': {e}", exc_info=True)
            self.errorMessage.emit(f"Не удалось импортировать файл: {e}")
            return False

    def _import_from_json(self, path: str) -> bool:
        ...
        if self.sqlite.import_from_json(path):
            ...
            self.successMessage.emit(f"База данных успешно импортирована из JSON. Создан бэкап: {os.path.basename(backup_db_path)}")
            return True
        else:
            self.errorMessage.emit("Выбранный JSON файл не содержит корректных данных или произошла ошибка импорта.")
            # Восстанавливаем бэкап, если импорт не удался
            if os.path.exists(backup_db_path):
                shutil.copy2(backup_db_path, self.db_path)
            return False

    def _import_from_zip(self, path: str) -> bool:
        """Восстанавливает базу данных и файлы из ZIP-архива."""
        db_folder = os.path.dirname(self.db_path)
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        
        # 1. Создание бэкапа текущего состояния
        backup_zip_path = os.path.join(db_folder, f"pre_import_backup_{timestamp}.zip")
        self.export_full_backup(backup_zip_path)
        logger.info(f"Создан полный бэкап текущего состояния: {backup_zip_path}")

        # 2. Распаковка нового архива во временную папку
        temp_dir = os.path.join(db_folder, f"temp_import_{timestamp}")
        ...
        try:
            ...
            new_db_path = os.path.join(temp_dir, os.path.basename(self.db_path))
            new_files_path = os.path.join(temp_dir, "attached_files")

            if not os.path.exists(new_db_path):
                self.errorMessage.emit("В архиве не найден файл базы данных (pro_database.db).")
                shutil.rmtree(temp_dir)
                return False

            # 3. Замена старых данных новыми
            # Закрываем текущее соединение с БД
            self.sqlite.close()

            # Удаляем старую БД и папку с файлами
            if os.path.exists(self.db_path):
                os.remove(self.db_path)
            
            current_files_path = os.path.join(db_folder, "attached_files")
            if os.path.exists(current_files_path):
                shutil.rmtree(current_files_path)

            # Перемещаем новые файлы и БД
            shutil.move(new_db_path, self.db_path)
            if os.path.exists(new_files_path):
                shutil.move(new_files_path, current_files_path)

            # 4. Перезагрузка бэкенда
            self.sqlite = SQLiteStorage(self.db_path) # Пересоздаем объект хранилища
            self.sqlite.set_security_manager(self.security)
            self._load_data() # Загружаем данные из новой БД
            
            self.successMessage.emit(f"Данные успешно восстановлены из архива. Предыдущее состояние сохранено в: {os.path.basename(backup_zip_path)}")
            return True

        except Exception as e:
            logger.error(f"Ошибка при восстановлении из ZIP: {e}", exc_info=True)
            self.errorMessage.emit(f"Ошибка при восстановлении: {e}. Восстанавливаем предыдущее состояние из бэкапа.")
            # Попытка восстановить из бэкапа, если что-то пошло не так
            self._import_from_zip(backup_zip_path) # Рекурсивный вызов для восстановления
            return False
        finally:
