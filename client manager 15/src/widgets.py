...
from PySide6.QtGui import QAction, QDoubleValidator, QFont, QFontMetrics, QGuiApplication
...
class ClientStatsWidget(QFrame):
    ...
    def init_ui(self, stats):
        ...
        self.val_sum = self.create_box("СУММА ЗАКАЗОВ", stats['total_sum'], "#00D1FF")
        self.val_paid = self.create_box("ВСЕГО ОПЛАЧЕНО", stats['total_received'], "#28A745")
        self.val_adv = self.create_box("АВАНСЫ", stats['total_advance'], "#FFD700")
        self.val_debt = self.create_box("ДОЛГ", stats['total_debt'], "#FF4B2B", check_debt=True)

    def create_box(self, title, value, color, check_debt=False):
        box = QFrame()
        box.setStyleSheet("""
            QFrame {
        ...
        # Value Label
        val_label = QLabel(self.format_money(value))
        val_label.setAlignment(Qt.AlignmentFlag.AlignCenter)
        
        text_color = color
        if check_debt:
            text_color = "#777777" if value <= 0 else "#FF4B2B"
            
        val_label.setStyleSheet(f"font-size: 20px; color: {text_color}; font-weight: bold; border: none; background: transparent;")
    ...
    def update_stats(self, stats):
        self.val_sum.setText(self.format_money(stats['total_sum']))
        
        self.val_paid.setText(self.format_money(stats['total_received']))
        
        self.val_adv.setText(self.format_money(stats['total_advance']))
        
        self.val_debt.setText(self.format_money(stats['total_debt']))
        debt_color = "#777777" if stats['total_debt'] <= 0 else "#FF4B2B"
        self.val_debt.setStyleSheet(f"font-size: 20px; color: {debt_color}; font-weight: bold; border: none; background: transparent;")
...
class AdaptiveDashLabel(QLabel):
    def __init__(self, value, color, is_money=True, parent=None):
        ...
        self.setMinimumWidth(50)  # Чтобы не схлопывался в 0
        self.update_display()

    def update_display(self):
        # 1. Подготовка текста
        if self.is_money:
            self.display_text = f"{self.value:,.0f}".replace(",", " ")
            self.digits_count = len(str(int(abs(self.value))))
        else:
            self.display_text = str(self.value)
...
class OrderWidget(QFrame):
    ...
    def show_settings_menu(self):
        ...
        # 5. Удалить заказ (существующая функция)
