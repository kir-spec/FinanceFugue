...
from PyQt6.QtWidgets import (
    ...
    QInputDialog, QGroupBox, QGridLayout  # Добавлен QGridLayout
)
...
class Order:
    ...
    @property
    def debt(self):
        return max(0.0, self.price - self.received)
    ...
    def add_payment(self, amount):
        ...
        payment_type = "аванс" if self.received - amount == 0 and self.advance == amount else "регулярный"
                ...
                for c_dict in data:
                    ...
                    for o in c_dict.get('orders', []):
                        ...
                        order = Order(
                            ...
                            received=o.get('received', o.get('advance', 0.0)),
                            created_at=o.get('created_at', ''),
                            deadline=o.get('deadline', ''),
                            status=o.get('status', 'В работе'),
                            files=files,
                            payment_history=o.get('payment_history', [])
                        )
                        orders.append(order)
...
class OrderWidget(QFrame):
    ...
    def init_ui(self):
        ...
        financial_layout.setContentsMargins(0, 0, 0, 0)
        financial_layout.setSpacing(10)
        
        cost_label = QLabel("Стоимость:")
        cost_label.setStyleSheet("color: #CCCCCC; font-size: 12px;")
        self.cost_edit = QLineEdit(str(self.order.price))
        self.cost_edit.setFixedWidth(100)
        self.cost_edit.textChanged.connect(self.sync_data)
        ...
        advance_label.setStyleSheet("color: #CCCCCC; font-size: 12px;")
        self.advance_edit = QLineEdit(str(self.order.advance))
        self.advance_edit.setFixedWidth(100)
        self.advance_edit.textChanged.connect(self.sync_advance)
        ...
        received_label.setStyleSheet("color: #CCCCCC; font-size: 12px;")
        self.received_label = QLabel(f"{self.order.received:.2f}")
        self.received_label.setStyleSheet("""
            color: #28A745;
            font-weight: bold;
            font-size: 12px;
            padding: 4px;
            ...
            background-color: #333333;
        """)
        self.received_label.setFixedWidth(100)
        
        debt_label = QLabel("Долг:")
        debt_label.setStyleSheet("color: #CCCCCC; font-size: 12px;")
        self.debt_label = QLabel(f"{self.order.debt:.2f}")
        self.update_debt_display()
        
        self.add_payment_btn = QPushButton("Оплачено")
        self.add_payment_btn.setFixedWidth(90)
        ...
        financial_layout.addWidget(cost_label, 0, 0)
        financial_layout.addWidget(self.cost_edit, 0, 1)
        financial_layout.addWidget(advance_label, 0, 2)
        financial_layout.addWidget(self.advance_edit, 0, 3)
        financial_layout.addWidget(received_label, 1, 0)
        financial_layout.addWidget(self.received_label, 1, 1)
        financial_layout.addWidget(debt_label, 1, 2)
        financial_layout.addWidget(self.debt_label, 1, 3)
        financial_layout.addWidget(self.add_payment_btn, 0, 4, 2, 1)
    ...
    def sync_data(self):
        try:
            self.order.price = float(self.cost_edit.text().replace(',', '.') or 0)
    ...
    def sync_advance(self):
        try:
            ...
            if new_advance < self.order.received:
                QMessageBox.warning(self, "Ошибка", "Аванс не может быть меньше уже полученной суммы")
                self.advance_edit.setText(str(self.order.advance))
                return
            
            self.order.advance = new_advance
            
            if not self.order.payment_history and self.order.advance > 0:
                self.order.received = self.order.advance
    ...
    def update_financial_display(self):
        self.received_label.setText(f"{self.order.received:.2f}")
        self.update_debt_display()
        self.update_payment_status()
        self.update_payment_progress()
        self.update_payment_button_state()
    ...
    def update_payment_status(self):
        ...
        elif self.order.received > 0:
            percentage = (self.order.received / self.order.price * 100) if self.order.price > 0 else 0
            self.payment_status.setText(f"⚠ Частично оплачено ({percentage:.1f}%)")
    ...
    def update_payment_progress(self):
        if self.order.price > 0:
            percentage = (self.order.received / self.order.price * 100)
            self.payment_progress.setText(f"Прогресс оплаты: {percentage:.1f}%")
    ...
    def update_payment_button_state(self):
        if self.order.debt <= 0:
            self.add_payment_btn.setText("Оплачено")
    ...
    def add_payment_dialog(self):
        ...
        amount, ok = QInputDialog.getDouble(
            self,
            "Добавить платеж",
            f"Введите сумму платежа:\n(Максимально: {self.order.debt:.2f} руб.)",
            min=0.01,
            max=self.order.debt,
            decimals=2
        )
        ...
        if ok and amount > 0:
            try:
                ...
                QMessageBox.information(
                    ...
                    f"Получено: {self.order.received:.2f} руб.\n"
...
class ProMusicCRM(QMainWindow):
    ...
    def update_dash(self):
        ...
        stats = [
            ...
            ("💰 АВАНСЫ", f"{total_advance:,.0f} ₽", "#FFFFFF"),
    ...
    def calculate_client_stats(self, client):
        ...
        total_received = sum(o.received for o in client.orders)
    ...
    def refresh_list(self):
        self.cl_list.clear()
        for client in self.clients:
            # Исправлено: теперь показываем только имя клиента
            item_text = client.name
            item = QListWidgetItem(item_text)
            item.setData(Qt.ItemDataRole.UserRole, client.id)
            # Убрана установка красного цвета для клиентов с долгами
    ...
    def add_order(self):
        ...
        def create_order():
            try:
                ...
                if advance > 0:
                    new_order.payment_history.append({
                        'type': 'аванс',
                        'amount': advance,
                        'date': datetime.now().strftime("%d.%m.%Y %H:%M"),
                        'remaining_debt': price - advance
                    })
