...
class GlobalFileManagerDialog(QDialog):
        ...
        for client in self.parent_app.clients:
            ...
            for order in client.orders:
                ...
                for file in order.files:
                    ...
                    icon = "📁" if getattr(file, 'is_folder', False) else "📄"
                    file_item.setText(0, f"{icon} {file.name}")
                    file_item.setText(1, file.path)
                    file_item.setText(2, "Папка" if getattr(file, 'is_folder', False) else "Файл")
    ...
    def show_context_menu(self, pos):
        ...
        menu = QAction("Удалить", self)
        menu.triggered.connect(self.delete_selected)
        
        from PySide6.QtWidgets import QMenu
        context = QMenu(self)
        context.addAction(menu)
        context.exec(self.tree.mapToGlobal(pos))
    ...
    def delete_selected(self):
        ...
        if not parent:
            # Это клиент
            QMessageBox.warning(self, "Внимание", "Удаление клиентов доступно только из главного окна")
            return
            
        grandparent = parent.parent()
        if not grandparent:
            # Это заказ
            QMessageBox.warning(self, "Внимание", "Удаление заказов доступно только из главного окна")
            return
